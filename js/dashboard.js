...
function initDashboard(role) {
    ...
    // financialWrapper already defined above in this scope for clarity elsewhere, 
    // but ensures it's reachable here if moved or needed.
    ...
    function showSection(section) {
        ...
        if (section === financialWrapper) section.style.display = 'block'; // Or whatever layout needed
    }
    ...
    // 7. Financial Reports Logic
    let financialData = [];
    function generateMockFinancialData() {
        const months = ["2025-01", "2024-12", "2024-11", "2024-10", "2024-09", "2024-08"];
        financialData = v_features.map((f, i) => {
            const props = f.properties || {};
            const priceStr = props.Precio || "45.000.000";
            const price = parseInt(String(priceStr).replace(/[^0-9]/g, '')) || 45000000;
            return {
                lote: props.Lote || (i + 1),
                monto: price,
                fecha: months[i % months.length] + "-" + (Math.floor(Math.random() * 28) + 1).toString().padStart(2, '0'),
                vendedor: ["Juan Pérez", "María Silva", "Roberto Gómez"][i % 3]
            };
        });
        financialData.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
    }

    let finChart;
    function renderFinancialReports() {
        if (financialData.length === 0) generateMockFinancialData();
        ...
        document.getElementById('fin-avg-price').innerText = '$ ' + (filtered.length ? Math.round(totalIncome / filtered.length) : 0).toLocaleString('es-CL');
        ...
        tbody.innerHTML = filtered.map(d => `
            <tr>
                ...
                <td style="padding: 10px;">Lote ${d.lote}</td>
                <td style="padding: 10px; text-align: right;">$ ${d.monto.toLocaleString('es-CL')}</td>
            </tr>
        `).join('');
    ...
    document.getElementById('btn-export-pdf').onclick = () => {
        ...
        const data = financialData.map(d => [d.fecha, "Lote " + d.lote, d.vendedor, "$ " + d.monto.toLocaleString('es-CL')]);
